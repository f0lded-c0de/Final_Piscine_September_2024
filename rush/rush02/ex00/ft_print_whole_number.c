...
/*   Updated: 2024/10/06 23:24:15 by bsamzun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_still_stuff_to_print(char *str, int rank)
{
	int	i;
...
void	ft_print_rank(int rank, char *str, char **dico, int *block)
{
	*block = 1;
	...
	else if ((rank % 3) == 2 && str[0] == '1')
	{
		...
		if (str[1] == '0' && rank > 3)
			write(1, " ", 1);
		ft_still_stuff_to_print(&str[1], rank);
		return ;
	}
	else if ((rank % 3) == 2)
	...
	if (ft_still_stuff_to_print(&str[1], rank) || rank > 3)
		write(1, " ", 1);
}
...
void	ft_print_whole_number(char *str, char **dico)
{
	...
	while (str[i])
	{
		...
		if ((rank % 3) == 1 && block != 0 && rank != 1)
		{
			...
			if (ft_still_stuff_to_print(&str[i + 1], 0))
				write(1, " ", 1);
		}
		i++;
	}
